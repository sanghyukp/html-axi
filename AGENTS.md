...
pnpm run build:skill    # Regenerate skills/ai-dev/SKILL.md from shared CLI guidance
...
The committed `skills/ai-dev/SKILL.md` is generated by `pnpm run build:skill`; `pnpm run check` fails if it drifts from the shared no-args home output.
...
- Runtime guidance strings (`src/cli.js`, `src/design-reference.js`, `src/playbooks.js`) own everything agents are told while using AI-DEV; the generated skill mirrors them via `src/skill.js`.
...
AI-DEV Editor is a CLI + local HTTP server that opens agent-generated HTML artifacts in a browser, lets the user annotate elements and selected text ranges or edit Mermaid diagrams as whiteboards, and ships that feedback back to the agent over a long-polling API.

### Process model

The CLI (`bin/ai-dev-axi.js` -> `src/cli.js`) spawns `ai-dev-axi server` as a **detached** background process (`src/cli.js:startServer`) and waits for `/health`, which returns `{ ok, app, version }`.
...
Server self-shutdown keys off live connections (browser SSE and agent polls), not session status, so the next `ai-dev-axi <file>` re-spawns a fresh server and adopts the session from `state.json` when it is still resumable; README's Server cleanup bullet owns the user-facing idle-timeout rules.
State lives at `~/.ai-dev/state.json` (`LAVISH_AXI_STATE_DIR`), shared across all projects and keyed by a sha256 prefix of the canonicalized file path - the canonical HTML path _is_ the identity, so the CLI never needs opaque session IDs (`src/session-store.js:sessionKey`).

### Request flow

1. `ai-dev-axi <file.html>` (`openCommand`) -> POST `/api/sessions` -> `SessionStore.upsertSession` -> the CLI opens the returned session URL in the browser.
   `--no-gate` sends `noGate`, returns a one-open URL with `?no-gate=1`, and leaves the stored canonical session URL unchanged.
   Sessions record who ended them: `ended_by: "user"` for browser-initiated ends (including the composer's send-and-end, which posts final prompts and the `endSession` flag to `/api/:key/prompts` atomically) and `ended_by: "agent"` for `ai-dev-axi end <file>`.
...
2. `GET /session/:key` serves a chrome page (`createChromeHtml`) whose sandboxed iframe points at `/artifact/:key/index.html`; the chrome client bootstraps from the `lavish-session` JSON script in the page.
   `extractArtifactHead` (`src/server.js`) adopts the artifact's own `<title>` and `<link rel="icon">` for the chrome tab, but only `data:`, `http(s):`, and protocol-relative icon hrefs - artifact-relative hrefs would not resolve against the chrome page - with fallback to the AI-DEV defaults.
...
7. `ai-dev-axi poll <file.html>` (`pollCommand`) hits `GET /api/poll`.
...
The opt-in attribute/meta (user-facing contract: README's Live reload bullet) switches the watcher to the artifact's directory, excluding `.git`, `node_modules`, `dist`, `build`, `.ai-dev-axi`.
...
Run `ai-dev-axi server --verbose` (or set `LAVISH_AXI_DEBUG=1`) to log session and watcher events to stderr when diagnosing wedges; detached server stdout/stderr is also appended to `server.log` in `LAVISH_AXI_STATE_DIR` for startup and crash diagnostics.
...
Absolute `file://` paths in non-inlined regions are redacted to `about:blank` so local paths do not leak into exports or hosted shares; the injected AI-DEV SDK is stripped; in-document fragment refs (`#a`, encoded `%23a`) are left alone; and inlined `</script>`/`</style>` are escaped so they can't break out.
...
The chrome's **Export standalone HTML** overflow-menu item `GET`s `/api/:key/export`; the CLI exposes the same transform as `ai-dev-axi export`, server-independently.
AI-DEV itself sets **no** `Content-Security-Policy` on any response (the sandboxed iframe relies on the `sandbox` attribute, not CSP), but author-set CSP meta tags are preserved and reported as export notices because they may still block exported inline assets.
...
The `home()` callback returns the rich object shown when the user runs `ai-dev-axi` with no arguments - the same TOON-serialized output that lands in the agent's optional `SessionStart` hook after `ai-dev-axi setup hooks`; top-level `--help` returns the same static guidance without dynamic sessions.
...
The bare-arg form (`ai-dev-axi some.html`) is normalized into `["open", "some.html"]` by `normalizeArgv`, which must let the SDK's `RESERVED_COMMANDS` (such as the built-in `update` self-updater) pass through untouched; otherwise the bare-arg rewrite turns `ai-dev-axi update` into `["open", "update"]` and the inherited reserved command never reaches `runAxiCli`.
...
- The SDK injected into artifacts lives in `src/artifact-sdk.js` and is wrapped by `createSdkJs`.
  ...
  The `ai-dev-axi design` Mermaid snippet chooses dark or light rendering from the effective artifact page background and re-renders when a page-theme or OS appearance change alters that appearance, so do not hardcode one Mermaid theme.
